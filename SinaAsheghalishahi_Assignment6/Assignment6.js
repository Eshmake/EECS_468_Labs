...
Program: EECS 468 Assignment 06
Description: JS program implements an HTTP remote-access file server via Node.js
Input: None
Output: A remote-access HTTP file server
Collaborators: NA
Other Sources: chatgpt.com and class slides 
*/

//NOTE: slides used for all code other than MKCOL
...
function urlPath(url) {
    //extract and decode path name
    let {pathname} = parse(url);
    //use resolve to create an absolute directory path
    let path = resolve(decodeURIComponent(pathname).slice(1));
    //check if path is outside the base directory
    if (path != baseDirectory && !path.startsWith(baseDirectory + sep)) {
        //if so, then throw 403 error
        throw {status: 403, body: "Forbidden"};
    }
    //if resolved path is not outside base directory, then return it
    return path;
}
